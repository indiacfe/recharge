...
import com.cfeindia.b2bserviceapp.dto.InsuranceBillDto;
...
public class InsuranceBillController {
	...
	private String XML_VIEW_NAME = "insuranceBillPay";

	@RequestMapping(value = "/doinsuranceBillPay", method = RequestMethod.POST)
	public ModelAndView doDataCardRecharge(@RequestBody String body) {
		
		ObjectMapper mapper = new ObjectMapper();
		InsuranceBillDto dto = null;
		TransactionTransportBean result = new TransactionTransportBean();
		result.setTransactionName("INSURANCE_BILL");
		FranchiseeCurrBal franchiseeCurrBal = null;
		double preBal = 0.0;
		String outcome = "";
		String userId = null;
		try {
			dto = mapper.readValue(body, InsuranceBillDto.class);
			...
			result.setRechargeType("INSURANCE");
		...
		return new ModelAndView(XML_VIEW_NAME, "insuranceBillPay", outcome);
	}
