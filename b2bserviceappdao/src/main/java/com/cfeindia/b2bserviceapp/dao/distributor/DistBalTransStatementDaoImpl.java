...
public class DistBalTransStatementDaoImpl implements DistBalTransStatementDao {
	...
	public List<DistributorBalanceTransferLog> getBalanceTransferStatement(
			Timestamp fromDate, Timestamp toDate, String distId) {
		long distributorId = CyberTelUtil.getStrInLong(distId);
		Criteria criteria = sessionFactory.getCurrentSession()
				.createCriteria(DistributorBalanceTransferLog.class)
	...
	@Override
	public List<DistributorBalanceTransferLog> getTransferedDetails(
			Timestamp fromDate, Timestamp toDate, Long distId) {
		Query query = sessionFactory
				.getCurrentSession()
				.createQuery(
						"FROM DistributorBalanceTransferLog AS dbtl WHERE  dbtl.id IN(SELECT  MAX(id)FROM DistributorBalanceTransferLog WHERE distId=:distId AND createdAt BETWEEN :sDate AND :eDate GROUP BY retailerDetail.userDetailId)");
		query.setLong("distId", distId);
		query.setTimestamp("sDate", fromDate);
		query.setTimestamp("eDate", toDate);
		return query.list();
	}

	@Override
	public Double getTotalAmount(Long distId, Timestamp startDate,
			Timestamp endDate) {
		Query query = sessionFactory
				.getCurrentSession()
				.createQuery(
						"SELECT SUM(transferAmount) AS total FROM DistributorBalanceTransferLog  WHERE distId=:distId AND createdAt BETWEEN :sDate AND :eDate");
		query.setLong("distId", distId);
